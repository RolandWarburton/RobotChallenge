...
class Parser {
    constructor(table) {
        ...
        this.eventEmitter.on("ROBOT", (robotIndex) => {
            ...
            // check its in bounds
            if (newRobotIndex > this.table.robots.length)
                return;
        ...
        this.eventEmitter.on("LEFT", () => {
            ...
            if (table.activeRobot)
                table.activeRobot.emitter.emit("LEFT");
        });
        ...
        this.eventEmitter.on("RIGHT", () => {
            ...
            if (table.activeRobot)
                table.activeRobot.emitter.emit("RIGHT");
        });
        ...
        this.eventEmitter.on("MOVE", () => {
            ...
            if (table.activeRobot)
                table.activeRobot.emitter.emit("MOVE");
        });
        ...
        this.eventEmitter.on("REPORT", () => {
            ...
            if (table.activeRobot)
                table.activeRobot.emitter.emit("REPORT");
        });
    }
    _getCommandType(line) {
        const args = line.split(" ");
        const cmd = args[0];
        let cmdType;
        switch (cmd) {
